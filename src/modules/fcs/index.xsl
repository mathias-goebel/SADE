...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fcs="http://clarin.eu/fcs/1.0" xmlns:sru="http://www.loc.gov/zing/srw/" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0"><!--
    ...
    <xsl:template match="/"><!--		
        ...
        <sru:scanResponse>
            ...
            <xsl:choose>
                <xsl:when test="$mode='subsequence'"><!--                    don't go descendants-axis, because of nested terms
    ...
    <xsl:template match="nodes[*]">
        ...
        <xsl:variable name="count-text" select="count($nodes/text()[.!=''])"/><!-- FIXME: this is not correct, because it counts individual text-nodes.
            ...
            <xsl:choose>
                <xsl:when test="$sort='text'">
                    <xsl:for-each-group select="$nodes" group-by=".//text()">
                        <xsl:sort select=".//text()" data-type="text" order="ascending"/><!--                        <xsl:sort select="count(current-group())" data-type="number" order="descending"/>-->
                        <sru:term>
                            <sru:value>
    ...
    </xsl:template><!-- if filter by default use only filtered data, except when sort=text and either filter mode = starts-with (only sensible result )
    ...
    <xsl:template match="sru:terms" mode="subsequence">
        <xsl:variable name="only-filtered" select="not($sort='text' and ($filter-mode='starts-with' or not(xs:integer($response-position) = 1)))"/><!-- position of the matching term within the index, if there is a filter -->
        ...
        <xsl:variable name="match-position" select="count(sru:term[.=$filtered[1]]/preceding-sibling::sru:term)"/><!--        <xsl:message><xsl:value-of select="$match-position" /></xsl:message>-->
        ...
        </xsl:message><!-- expect ordered data 
        ...
        </xsl:message><!-- 		<xsl:variable name="count-items" select="count($filtered)" />-->
        <xsl:copy><!--            <xsl:copy-of select="@*" />--><!--			<xsl:attribute name="count_items" select="if (xs:integer($count-items) > xs:integer($max-items)) then $max-items else $count-items" /> --><!--            <xsl:message>cnt:<xsl:value-of select="count($ordered/*)"/>-match:<xsl:value-of select="$match-position"/>-start:<xsl:value-of select="$effective-start-item"/>-end:<xsl:value-of select="$effective-end-item"/>
                </xsl:message>-->
    ...
    <xsl:template match="sru:term">
        ...
        <xsl:copy>
            <xsl:copy-of select="*[not(local-name()='extraTermData')]"/><!-- <xsl:attribute name="pos" select="position()"/> -->
            <sru:extraTermData><!--                pass pre-existing sru:extraTermData through-->
                <xsl:copy-of select="sru:extraTermData/*"/>
                <fcs:position>
                    <xsl:value-of select="position() + $start-pos"/>
                </fcs:position><!-- dig deaper -->
