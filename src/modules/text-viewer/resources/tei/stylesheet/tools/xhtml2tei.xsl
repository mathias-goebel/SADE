...
<xsl:stylesheet xmlns:dt="uuid:C2F41010-65B3-11d1-A29F-00AA00C14882" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" exclude-result-prefixes="h v o w dt" version="1.0">
...
created by converting an HTML file to TEI XML on TODAYSDATE.</p>
    ...
    <xsl:template match="h:body">
        <body>
            <xsl:apply-templates/><!--This code is intended to balance the div tags created by headers with --><!--     the correct number of /divs. At the moment it fails if there are --><!-- Hn tags inside td's, which we are intending to make cell labels -->
   ...
   diff=<xsl:value-of select="$difference"/>
            </xsl:comment><!-- test call of for loop to handle closure of divs
got to find a way to set a trigger for closing div if no headers -->
            ...
            </xsl:call-template><!-- old div closing code was here-->
        </body>
    </xsl:template>
    <xsl:template match="h:h1|h:h2|h:h3|h:h4"><!--headers inside td's are labels-->
        <xsl:choose>
...
--><!-- test call of for loop to handle this ... -->
    ...
    <xsl:template name="for-loop"><!-- simplified from tidwell p86. only does step up or down until -->
