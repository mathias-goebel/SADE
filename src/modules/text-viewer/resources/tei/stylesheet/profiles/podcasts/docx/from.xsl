...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:iso="http://www.iso.org/ns/1.0" xmlns:tbx="http://www.lisa.org/TBX-Specification.33.0.html" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture" xmlns:media="http://search.yahoo.com/mrss/" xmlns:mml="http://www.w3.org/1998/Math/MathML" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:w10="urn:schemas-microsoft-com:office:word" xmlns:rel="http://schemas.openxmlformats.org/package/2006/relationships" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:creativeCommons="http://backend.userland.com/creativeCommonsRssModule" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math" xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:ve="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:wne="http://schemas.microsoft.com/office/word/2006/wordml" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:itunes="http://www.itunes.com/dtds/podcast-1.0.dtd" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:teidocx="http://www.tei-c.org/ns/teidocx/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0" exclude-result-prefixes="cp ve o r m v wp w10 w wne mml media       tbx pic rel a creativeCommons       itunes tei teidocx xs iso"><!-- import base conversion style -->
        ...
        <xsl:analyze-string select="$filename" regex="([^/]+)/([^/]+)/([^\.]+)\.?.*">
            <xsl:matching-substring>
                ...
                <xsl:for-each select="document($lookup)">
                    <xsl:choose>
                        <xsl:when test="count(//item[contains(enclosure/@url,$file)])=1">
                            <xsl:for-each select="//item[contains(enclosure/@url,$file)]">
                                <teiHeader>
                                    <fileDesc>
                                        <titleStmt>
                                            <title>
                                            ...
                                            <author>
                                                <xsl:for-each select="media:credit">
                                                    <xsl:value-of select="."/>
                                                    <xsl:choose>
                                                        <xsl:when test="count(following-sibling::media:credit)=1">
                                                            <xsl:text> and </xsl:text>
                                                        </xsl:when>
                                                        <xsl:when test="following-sibling::media:credit">
                                                            <xsl:text>, </xsl:text>
                                                        </xsl:when>
                                                    </xsl:choose>
                                                </xsl:for-each>
                                        ...
                                        <editionStmt>
                                            <edition>
                                                <date>
                                                    <xsl:call-template name="convertFrom822To8601">
                                                        <xsl:with-param name="myDate" select="pubDate"/>
                                                    </xsl:call-template>
                                                </date>
                                            </edition>
                                        </editionStmt>
                                        <publicationStmt>
                                            ...
                                            <idno type="licence">
                                                <xsl:value-of select="creativeCommons:license"/>
                                            </idno>
                                            <publisher>
                                    ...
                                    <profileDesc>
                                        <textClass>
                                            <keywords scheme="http://podcasts.ox.ac.uk">
                                                <list>
                                                    <xsl:for-each select="tokenize(itunes:keywords,',')">
                                                        <item>
                                                            <xsl:value-of select="."/>
                                                        </item>
                                                    </xsl:for-each>
                        ...
                        <xsl:otherwise>
                            <xsl:message terminate="yes">Error finding <xsl:value-of select="$file"/> in <xsl:value-of select="$lookup"/> (from <xsl:value-of select="$filename"/>)</xsl:message>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:for-each>
            </xsl:matching-substring>
            <xsl:non-matching-substring>
                <xsl:message terminate="yes">Error finding a valid full name in <xsl:value-of select="regex-group(0)"/>
                </xsl:message>
            </xsl:non-matching-substring>
        </xsl:analyze-string>
    </xsl:template>
            ...
            <xsl:for-each select="tei:body">
                <xsl:copy>
                    <xsl:variable name="body">
                        <xsl:for-each-group select="*" group-starting-with="tei:p[contains(.,'START AUDIO') or            contains(.,'END AUDIO')]">
                            <xsl:choose>
                    ...
                    <xsl:for-each select="$body">
                        <xsl:for-each-group select="*" group-starting-with="tei:p[tei:speaker]">
                            <sp>
                                <xsl:for-each select="current-group()">
    ...
    <xsl:template match="@*|text()|comment()|processing-instruction()" mode="findsp">
        <xsl:copy-of select="."/>
    </xsl:template>
    <xsl:template match="*" mode="findsp">
        <xsl:copy>
            <xsl:apply-templates select="*|@*|processing-instruction()|comment()|text()" mode="findsp"/>
        </xsl:copy>
    </xsl:template>
