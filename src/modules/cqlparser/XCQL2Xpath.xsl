...
-->
    ...
    <xsl:template match="/">
        ...
        </xsl:call-template><!-- <xsl:call-template name="message">
            <xsl:with-param name="msg"><xsl:copy-of select="doc($mappings-file)"/>
            </xsl:with-param>
        </xsl:call-template>-->
        <xsl:variable name="xpath-query">
            <xsl:apply-templates/>
        </xsl:variable><!-- moving the base-elem out into xquery        
<xsl:value-of select="concat($xpath-query,'/', $base-elem)"/>-->
        <xsl:value-of select="$xpath-query"/>
    </xsl:template>
    <xsl:template match="triple"><!--special handling for collection (=context) index, disabled for now,
    ...
    <xsl:template match="searchClause">
        ...
        </xsl:variable><!--        <xsl:value-of select="$resolved_index" />-->
    ...
    </xsl:template><!--   aim is: special handling of collection-filter (as separate parameter) --><!--<xsl:template match="searchClause" mode="collection">
        <xsl:text>&<xsl:value-of select="$context-param" />=</xsl:text>
        <xsl:value-of select="term"/>
        </xsl:template>-->
    <xsl:template match="index"><!-- <xsl:variable name="ix_xpathed" select="translate($ix_resolved, '.', '/')" ></xsl:variable> --><!--        <xsl:variable name="ix_xpathed" select="my:index2xpath(.)"/>-->
    ...
    <xsl:template match="relation">
        ...
        <xsl:param name="index-type" select="if (exists($index/index/@index-type)) then xs:string($index/index/@index-type) else  '' "/><!--        <xsl:message>relation index:<xsl:value-of select="$index/index/@use"/></xsl:message>-->
        <xsl:variable name="sanitized_term"><!-- select="replace($term,' ','+')"/ -->
        ...
        <xsl:choose>
            ...
            <xsl:when test="($index-type = ('fulltext','ft','lucene') or value='any' or preceding-sibling::index='*' or preceding-sibling::index='cql.serverChoice')"><!--  use full-text querying: [ft:query(., "language")] -->
                <xsl:choose><!--                    <xsl:when test="starts-with($term,'%22') or starts-with($term,'"') ">-->
                    <xsl:when test="contains($sanitized_term, ' ')">
                        <xsl:value-of select="concat('ft:query(', $match-on, ', ')"/><!--                        <xsl:text>ft:query(.,<phrase></xsl:text>--><!-- <xsl:text><phrase></xsl:text>
        ...
        <xsl:text>]</xsl:text>
    </xsl:template><!-- based on $as-param - return either the resolve xpath-string or the matching index-element -->
    <xsl:template name="resolve-index">
        <xsl:param name="ix" select="."/><!-- reverting the "escaping" of whitespaces in indices with datcat-name -->
        <xsl:variable name="ix_string" select="replace($ix/text(),'_',' ')"/><!--<xsl:call-template name="message">
            <xsl:with-param name="msg">resolve index:<xsl:value-of select="$ix"/>
            </xsl:with-param>
        </xsl:call-template>--><!--$resolved-index := if (exists($index-map)) then $index-map/text()
            else if (exists($repo-utils:mappings//index[xs:string(@key) eq $index])) then
            $repo-utils:mappings//index[xs:string(@key) eq $index]
            else $index--><!--        <xsl:variable name="resolved_ix">--><!--        <xsl:copy-of select="$ix"></xsl:copy-of>-->
        <xsl:choose>
            <xsl:when test="exists($context-mapping/index[xs:string(@key) eq  $ix_string])"><!--                <xsl:copy-of select="$context-mapping/index[xs:string(@key) eq  $ix_string]"></xsl:copy-of>-->
                <xsl:apply-templates select="$context-mapping/index[$ix_string eq xs:string(@key)]" mode="copy"/>
            </xsl:when><!-- if no contextual mapping, try in default, than whole map -->
            ...
            <xsl:when test="exists($mappings//index[$ix_string eq xs:string(@key)])"><!--               <xsl:copy-of select="$mappings//index[$ix_string eq xs:string(@key)]" />
        ...
        </xsl:choose><!--        </xsl:variable>--><!--<xsl:copy-of select="$resolved_ix"></xsl:copy-of>
    ...
    </xsl:template><!-- index-resolution/mapping disabled    
        <xsl:choose>
